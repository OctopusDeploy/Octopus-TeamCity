    ...
    try (final OctopusTeamCityStack stack = SharedStack.full()) {
      ...
      PlaywrightUi.withLoggedInPage(
          ...
          page -> {
            ...
            // TeamCity lazy-loads the runner-parameters form (our controller-served JSP) via AJAX
            // after
            // the page loads, so wait for it to settle and for our dropdown to appear before
            // asserting.
            ...
            // The selected connection defines a space ("Default"), so the step's Space field is
            // hidden
