...
class OctopusBuildInformationRunTypeValidationTest {
  private static final OctopusConstants C = new OctopusConstants();

  private Collection<String> validate(final Map<String, String> p) {
    final OctopusBuildInformationRunType runType =
        new OctopusBuildInformationRunType(
            mock(RunTypeRegistry.class), mock(PluginDescriptor.class));
  ...
  private Map<String, String> withMandatoryNonCredentialFields(final Map<String, String> p) {
    p.put(C.getPackageIdKey(), "MyPackage");
    p.put(C.getPackageVersionKey(), "1.0.0");
    return p;
  }
