...
class OctopusDeployReleaseRunTypeValidationTest {
  private static final OctopusConstants C = new OctopusConstants();

  private Collection<String> validate(final Map<String, String> p) {
    final OctopusDeployReleaseRunType runType =
        new OctopusDeployReleaseRunType(mock(RunTypeRegistry.class), mock(PluginDescriptor.class));
  ...
  private Map<String, String> withMandatoryNonCredentialFields(final Map<String, String> p) {
    ...
    p.put(C.getReleaseNumberKey(), "1.0.0");
    p.put(C.getDeployToKey(), "Production");
    return p;
  }
