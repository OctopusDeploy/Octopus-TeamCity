...
class CommandHelperTest {
    ...
    void createReleaseCommandWithDeploy() {
        ...
        assertThat(login).contains("login", "--server", "https://octo.example", "--api-key", "API-KEY-123", "--no-prompt");

        String[] deploy = CommandHelper.deployRelease(params, null);
        assertThat(deploy).contains("release", "deploy", "--project", "MyProject", "--environment", "EnvA", "--output-format", "json", "--no-prompt");
    ...
    void deployReleaseCommandsSequence() {
        ...
        String[] deploy = CommandHelper.deployRelease(params, null);
        assertThat(deploy).contains("release", "deploy", "--project", "AnotherProject", "--environment", "EnvX", "--environment", "EnvY");
    }
    ...
    void packNugetPackageCommand() {
        ...
        assertThat(packCommandBuilder.buildCommand()).containsExactly(
                "package",
                "nuget",
                "create",
                "--id",
    ...
    void packZipPackageCommand() {
        ...
        assertThat(packCommandBuilder.buildCommand()).contains(
                "package",
                "zip",
                "create",
                "--id",
    ...
    void pushPackageUploadWithDefaultOverwrite() throws Exception {
        ...
        PushPackageBuildProcess proc = new PushPackageBuildProcess(mock(AgentRunningBuild.class), mock(BuildRunnerContext.class), null);
        ...
        assertThat(pushPackageCommand.buildCommand()).contains(
                "package",
                "upload",
                "--package",
                f.getAbsolutePath(),
