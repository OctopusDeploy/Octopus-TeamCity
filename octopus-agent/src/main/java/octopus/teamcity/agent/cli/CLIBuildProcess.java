...
public abstract class CLIBuildProcess implements BuildProcess {
    ...
    protected CLIBuildProcess(@NotNull AgentRunningBuild runningBuild, @NotNull BuildRunnerContext context) {
    ...
    private void extractOctopusBinary() throws RunBuildException {
        ...
        try {
            ...
            extractedTo = new File(tempDirectory, String.format("octopus-temp/%s/%s", octopusVersion, osFolder));
    ...
    private void startOctopus(final OctopusCommandBuilder command) throws RunBuildException {
        ...
        logger.message("Running command:   octopus " + StringUtils.arrayToDelimitedString(userVisibleCommand, " "));
        logger.progressMessage(getLogMessage());

        try {
            final ArrayList<String> arguments = new ArrayList<>();
            arguments.add(new File(extractedTo, String.format("/%s", octopusBinaryName)).getAbsolutePath());
            ...
            standardOutput = new Output.ReaderThread(process.getInputStream(), line -> {
